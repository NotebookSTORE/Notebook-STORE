...
class WalletFrag : Fragment(), View.OnClickListener, KodeinAware,
    ...
    override fun onCreateView(
        ...
        profileVM.redeemWalletPointsState.observe(viewLifecycleOwner, redeemObserver)
        ...
        if (arguments != null) {
            ...
            fragmentWalletBinding.tvAvailableBalanceText.text =
                "Available Balance ₹ ${walletArgs.walletAmount}"
        }
    ...
    private val redeemObserver = Observer<ActivityState> {
        when (it) {
            is ProfileVM.RedeemPointsSuccessState -> {
                showErrorView(it.message)
            }
            is ProfileVM.RedeemPointsErrorState -> {
                showErrorView(it.message)
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        paymentLiveData.observe(viewLifecycleOwner, Observer {
            if (it != null) {
                val paymentMethodDirections: WalletFragDirections.ActionWalletFragToPaymentCODSuccessScreen =
                    WalletFragDirections.actionWalletFragToPaymentCODSuccessScreen(
                        it.orderID,
        ...
        fragmentWalletBinding.btnRedeem.setOnClickListener(this)
    }

    private fun showRedeem() {
        fragmentWalletBinding.btnRedeem.visibility = View.VISIBLE
        fragmentWalletBinding.tvRedeemHint.visibility = View.VISIBLE
    }

    private fun hideRedeem() {
        fragmentWalletBinding.btnRedeem.visibility = View.GONE
        fragmentWalletBinding.tvRedeemHint.visibility = View.GONE
    }
    ...
    private fun showErrorView(msg: String) {
        ...
        fragmentWalletBinding.clErrorView.startAnimation(
    ...
    override fun onClick(v: View?) {
        when (v) {
                    ...
                    if (userData != null) {
                        if (!userData!!.phone.isNullOrEmpty()) {
                            if (notebookPrefs.isVerified == 1) {
                                val addWallet =
                                    AddWallet(amount.toFloat(), userData!!.id, userData!!.token!!)
                                profileVM.addWalletFromServer(addWallet)
                            } else {
                                errorToastTextView.text =
                                    "Please verify your phone number for proceed to payment"
                                errorToast.show()
                                navController.navigate(R.id.addDetailFrag)
                            }
                        } else {
                            errorToastTextView.text =
                                "Please update your phone number for proceed to payment"
                            errorToast.show()
                            navController.navigate(R.id.addDetailFrag)
                        }
                    } else {
            ...
            fragmentWalletBinding.btnRedeem -> {
                showRedeemDialog()
            }
        }
    }
...
//        super.onActivityResult(requestCode, resultCode, data)
        ...
        if (resultCode == Activity.RESULT_OK) {
            Log.e(
                "CFToken",
                "ReqCode :  :: ${CFPaymentService.PARAM_CUSTOMER_EMAIL}" + CFPaymentService.REQ_CODE
            )
        }
                ...
                for (key in bundle.keySet()) {
                    if (bundle.getString(key) != null) {
                        ...
                        if (txtStatus.equals("SUCCESS")) {
                            val afterPaymentRawData = AfterPaymentRawData(
                                1,
                                1,
                                userData!!.token!!,
                                userData!!.id,
    ...
    private fun doPayment(
        ...
        CFPaymentService.getCFPaymentServiceInstance().doPayment(
            mActivity, dataSendMap, cfToken, "PROD",
            "#1979BC", "#FFFFFF", false
        )
    }
    ...
    private fun showRedeemDialog() {
        context?.let {
            AlertDialog.Builder(it)
                ...
                .setMessage(R.string.redeem_message).setPositiveButton(
                    R.string.yes
                ) { _, _ ->
                    userData?.id?.let {
                        profileVM.redeemWalletPoints(it)
                    }
