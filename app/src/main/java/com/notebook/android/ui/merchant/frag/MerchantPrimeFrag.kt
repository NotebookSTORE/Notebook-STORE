...
import com.notebook.android.databinding.FragmentMerchantPrimeBinding
...
class MerchantPrimeFrag : Fragment(), KodeinAware, MerchantBenefitListener {

    private lateinit var merchantPrimeBinding:FragmentMerchantPrimeBinding
    ...
    override fun onCreateView(
        ...
        merchantPrimeBinding = DataBindingUtil.inflate(inflater,
            R.layout.fragment_merchant_prime, container, false)
        merchantPrimeBinding.lifecycleOwner = this
        merchantVM.merchantBenefitListener = this
        setViewMoreTextClickable()

        //success toast layout initialization here....
        val successToastLayout:View = inflater.inflate(R.layout.custom_toast_layout,
            merchantPrimeBinding.root.findViewById(R.id.custom_toast_layout) as? ViewGroup)
        ...
        val errorToastLayout:View = inflater.inflate(R.layout.error_custom_toast_layout,
            merchantPrimeBinding.root.findViewById(R.id.custom_toast_error_layout) as? ViewGroup)
        ...
        return merchantPrimeBinding.root
    }
    ...
    private fun setupRecyclerView(merchantBenefitData:List<MerchantBenefit>){
        ...
        merchantPrimeBinding.recViewMerchantPrimeBenefits.apply {
    ...
    private fun setViewMoreTextClickable(){
        val ssLoginText = SpannableString(resources.getString(R.string.strViewMore))
        ssLoginText.setSpan(
            ForegroundColorSpan(Color.parseColor("#ffffff")),
            0, ssLoginText.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        ssLoginText.setSpan(StyleSpan(Typeface.BOLD), 0, ssLoginText.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        val spanRegNow =  object : ClickableSpan() {
            override fun onClick(widget: View) {
                val navController = Navigation.findNavController(widget)
                navController.navigate(R.id.notebookPdfPage)
            }
        ...
        ssLoginText.setSpan(spanRegNow, 0, ssLoginText.length, Spanned.SPAN_INCLUSIVE_EXCLUSIVE)
        merchantPrimeBinding.tvViewMore.movementMethod = LinkMovementMethod.getInstance()
        merchantPrimeBinding.tvViewMore.text = ssLoginText
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        merchantVM.getMerchantBenefitData(Constant.MERCHANT_PRIME_BENEFIT)
            .observe(viewLifecycleOwner, Observer {
                if (it != null){
                    val merchBenefitFirst = it[0]
                    val merchBenefitLast = it[it.size-1]
                    merchantPrimeBinding.tvBenefitPrimeMerchant.text = Html.fromHtml(merchBenefitFirst.title).toString()
                    merchantPrimeBinding.tvPrimeMerchantDesc.text = Html.fromHtml(merchBenefitFirst.description).toString()
                    merchantPrimeBinding.tvWhySellOnNotebook.text = Html.fromHtml(merchBenefitLast.title).toString()
                    merchantPrimeBinding.tvWhySellOnNotebookDesc.text = Html.fromHtml(merchBenefitLast.description).toString()
